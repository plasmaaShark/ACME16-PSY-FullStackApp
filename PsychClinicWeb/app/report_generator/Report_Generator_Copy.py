...
image_path = os.path.join(current_directory, 'static', 'images', 'wsu_banner.png')
...
def create_report():
    ...
    for i in range (1, 2):
        ...
        df = Graph_Generator.pd.read_csv(csv_file_path)
        idxS = 0-i
        ...
        stuff = [list(data['Temperament'].values())[0:2], list(data['Temperament'].keys())[0:2]]
        ...
        send_mail('shaylinsmith07@gmail.com')
        plt.close('all')


def title_page(my_path, pdf, info):
    pdf.add_page()
    ensure_directory_exists(os.path.join(current_directory, 'static', 'images'))

    pdf.image(image_path, 0, 0, WIDTH)
    pdf.image(os.path.join(current_directory, 'static', 'images', 'Washington-State-University-logo.png'), 0, HEIGHT / 2 - 100, WIDTH)
    pdf.image(os.path.join(current_directory, 'static', 'images', 'buffer.png'), 0, HEIGHT / 2 + 1, WIDTH)
...
def goals_bar_graphs(my_path, pdf, data, descriptions, titles):
    ...
    for key, value in data.items():
        ...
        if counter == 2:
            ...
            pdf.image(os.path.join(current_directory, 'static', 'images', f"{holder}_Scaling.png"), 100, counter * 96 + 86, WIDTH / 2)
            pdf.add_page()
            PDF_Generator.section_headers(pdf, 'Personal Goals')
            counter = 0
        else:
            if counter != 0:
                ...
                pdf.image(os.path.join(current_directory, 'static', 'images', f"{holder}_Scaling.png"), 100, 94, WIDTH / 2)
                counter += 1
...
def rssm_bar_graphs(my_path, pdf, data, names, titles):
    ...
    for key, value in data.items():
        ...
        if counter == 2:
            ...
            pdf.image(os.path.join(current_directory, 'static', 'images', 'RSSM_Scaling.png'), 118, counter * 96 + 86, WIDTH / 2 - 20)
            pdf.add_page()
            PDF_Generator.section_headers(pdf, 'Relational Schema Psychological Need Scale')
            counter = 0
        else:
            if counter != 0:
                ...
                pdf.image(os.path.join(current_directory, 'static', 'images', 'RSSM_Scaling.png'), 118, 94, WIDTH / 2 - 20)
                counter += 1
