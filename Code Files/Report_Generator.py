...
def create_report():
    ...
    # Todo: Below code chunk to create radar temporarily commented out while error persists
    # print('----------------------------------------')
    ...
    # Graph_Generator.create_radar(pdf, my_path, data['RadarRSSM'], data['RadarRSSMName']) # TODO: Error: program stops prematurely with message 'StopIteration' in console. Possible problem with names used to index into 'data'
...
def title_page(my_path, pdf, info):
    pdf.add_page()
    pdf.image(my_path + "/images/wsu_banner_v2.png", 0, 0, WIDTH)
    # pdf.image(my_path + "/images/Washington-State-University-logo.png", 0, HEIGHT/2 - 100, WIDTH) # Removed WSU logo
...
def goals_bar_graphs(my_path, pdf, data, descriptions, titles):
    pdf.add_page()
    PDF_Generator.section_headers(pdf, 'Your Personal Goals')
    counter = 0
